...
在AI辅助主题开发页面,**Sam看到自己创建的主题以气泡图形式呈现**。通过**外围气泡半径可直观查看主题模式和规模（DG5）**。点击大主题气泡内的小气泡时,系统会在左侧面板动态显示相关编码和引文。

![alt text](image.png)。

色彩编码帮助识别重复出现的编码,使主题审阅过程更直观,这种结合交互性的数据可视化方式使得sam对已开发主题的理解和检验更加直观。
...
检索增强生成（RAG）技术通过动态整合外部非参数化知识源（Lewis等，2020；Gao等，2023b），已成为提升LLM在知识密集型任务中表现的有效方案。

传统RAG系统通过**单一检索-生成范式能高效处理简单单跳问答任务**（Ye等2024；Roy等2024），**但在需要整合多文档分散信息的多跳问答（MHQA）场景中表现欠佳**（He等2024；Trivedi等2022a；Jiang等2023b）。针对MHQA的挑战，**最新研究采用多轮检索与迭代优化策略逐步收集筛选信息**，有效融合多源知识显著增强了大语言模型的多跳推理能力（Tang和Yang 2024；Yang等2024b；Teng等2025）。然而**这类方法常存在推理路径规划效率低下与信息挖掘深度不足的问题**。

为寻找最优推理路径，部分研究**引入蒙特卡洛树搜索（MCTS）等复杂算法（Dong等2024；Li等2024），但伴随显著计算开销。此外模型在解决子查询时倾向提取直接答案，忽视对最终解决方案至关重要的潜在线索（Ye等2025；Wang等2024a）**。最新研究通过**引入重排序器、决策器等外部组件提升检索精度（Glass等2022；Jeong等2024），但往往以增加系统复杂度和降低可解释性为代价**（Su等2025）。

本文提出了一个迭代框架——递归评估与自适应规划（REAP），旨在提升检索增强生成（RAG）的性能。其核心观点是：**将复杂查询逐步分解为子查询可能导致推理僵局，而缺失或不准确的事实会偏离正确推理轨迹（Zhang等，2025）。**为保持全程推理的连贯性与可靠性，我们明确维护两个关键知识集：结构化子任务和事实，作为指导推理的基础。具体来说：
...
1. 提出双模块框架REAP：全局感知的SP模块动态引导推理轨迹，FE模块持续用可靠事实丰富知识库，形成增强推理能力的正向循环。
2. 通过多任务微调建立统一任务范式，实现从数据丰富的常规规划到数据稀缺的关键重规划的知识迁移，显著提升框架鲁棒性与策略智能。
3. 将简单与复杂推理分离至SP的不同子模块，允许用轻量级模型替换简单子模块以提高效率而不牺牲性能。
4. 在多个多跳问答数据集上的大量实验验证了REAP的有效性与鲁棒性。

### 相关工作

**传统RAG**框架通过引入外部知识源，旨在缓解大语言模型中知识过时和幻觉等问题。早期代表性方法如标准RAG采用静态的"检索-生成"范式来提高生成回答的事实准确性。

虽然这些方法在问答任务中保证了准确性，但由于缺乏动态交互和推理机制，其在复杂推理任务（尤其是多跳推理）中的效果仍然有限。

为解决这些问题，研究者从多角度改进单轮RAG以更好地支持复杂推理：

- 检索前采用查询增强和重写优化信息对齐（Gao等人，2023a；Ma等人，2023；Chan等人，2024）；
- 检索过程中引入基于置信区间的动态策略以减少冗余计算并提升效率（Jiang等人，2023b；Su等人，2024；Deng等人，2025）；
- 检索后通过端到端训练（Shi等人，2023）或后处理模块（Glass等人，2022；Jiang等人，2023a；Kim等人，2024）精炼检索内容以提升生成质量。

尽管存在这些改进，传统单轮检索RAG在处理复杂多跳推理任务时的固有缺陷，促使学界聚焦迭代式检索-生成架构。

迭代式RAG框架通过多轮交替检索与生成，以情境敏感的自适应方式逐步合成证据并构建推理链（Ram等人，2023；Yang等人，2024a）。

其核心策略是**问题分解：将复杂问题拆分为子查询，为每个子查询检索支撑证据，最后整合子答案以实现多跳推理**（Xu等人，2024；Shi等人，2024）。

为推进推理规划进程，部分研究引入**蒙特卡洛树搜索等算法模拟评估不同推理分支并确定最优策略**（Dong等人，2024；Li等人，2024）。

另一类研究将思维链提示与多轮检索结合，逐步构建最终答案（Yao等人，2023；Press等人，2022；Wang等人，2024b）。代表方法如IRCoT（Trivedi等人，2022a）和Iter-RetGen（Shao等人，2023）会**动态生成子查询、检索相关文档，并在生成过程中迭代优化推理轨迹**。

另一些则依赖**强化学习框架基于交互反馈学习检索与生成策略**（Jin等人，2025a；Song等人，2025）。**但这些方法在全局规划和事实提取方面仍存在不足，易导致脆弱且不连贯的推理轨迹**。

相较于现有方法，本文提出的REAP框架通过SP与FE之间的递归反馈循环，显著提升了多跳推理任务中的推理能力与鲁棒性。

### 基础

在多跳场景中，可以将迭代式RAG任务定义如下：

给定查询Q和一个外部语料库C，目标是生成一个事实性答案A。

该任务通常被实现为一个迭代推理过程，其中在每一步t，会生成一个子查询$q_t$以收集特定信息片段。随后，检索函数$Retriever(·)$基于该子查询获取一组相关文档$D_t ⊂ C$。

$$ Dt = Retriever(q_t; C) $$

一个由θ参数化的LLM（大语言模型）$M_θ$会处理这些文档，通常是基于之前交互的结果$H_{t-1}= \{Q, (q1, A1), . . . , (q_{t−1}, A_{t−1})\}$并生成子回答$A_t$。

$$A_t = M_θ (GenerateSubAnswer | Q, D_t, H_{t−1})$$

之后，历史记录加入新对(q_t,A_t)。并且重复该过程直到满足终止条件。最后，一个合成函数会根据完整的历史交互记录$H_final$生成答案A。
...
REAP框架将多跳推理过程从线性流程重构为动态的、状态驱动的循环。如下图所示，该架构围绕上述核心原则设计，即显式地将规划与事实收集解耦，以增强鲁棒性和可解释性。它主要由两个在递归循环中协同工作的模块组成：子任务规划器（SP）和事实提取器（FE）。

整体工作流程如下：

给定一个复杂查询Q，初始分解器首先生成一个结构化任务计划$P₀ = {p₁, p₂, ..., p_N}$。每个子任务$pᵢ ∈ P₀$是一个元组$(idᵢ, qᵢ, depsᵢ)$，分别表示其唯一标识符、查询和依赖关系。任务计划$P_0$与初始为空的事实列表$F₀ = ∅$共同定义了框架的初始状态。

REAP的核心是一个迭代过程，直到满足终止条件为止。在每一步t：

1. SP作为策略核心，分析当前状态(Pₜ₋₁, Fₜ₋₁)以确定一组可执行的下一步动作$Actionsₜ$。

2. 对于每个子任务$pᵢ ∈ Actionsₜ$，FE执行检索和基于事实的推理，生成一个新的事实对象$fᵢ$。

3. 更新事实列表：$Fₜ = Fₜ₋₁ ∪ \{f₁, f₂, ...\}$。随后，SP接收此反馈以生成后续计划$Pₜ$。

这种递归循环（SP指导FE，而FE的发现为SP的下一个规划周期提供信息）持续进行，直到计划完全解决。最后，合成器通过基于最终全面事实列表F_final和原始查询Q进行推理，生成结论性答案A：

$$A = M_θ(Synthesize | Q, F_{final})$$

#### 子任务规划器（SP）

作为REAP框架的战略核心，SP旨在克服短视的逐步推理常导致的局部僵局或死锁问题。通过接收并动态维护完整的初始任务计划P₀，SP始终保持着全局视角。其关键创新在于状态感知的模块化设计：根据事实抽取器（FE）返回的新事实fₜ的完成度lₜ，SP将任务分派至两个专用子模块之一：

- 计划更新器​

    处理理想推理场景（如lₜ为DirectAnswer时），通过确定性规则更新任务计划，主要功能包括：
...
- 重规划器​
    ...
    应对非理想场景（如lₜ为PartialClue或Failed），作为关键推理恢复机制，其分层职责为：

    - 实用充分性评估：首先判断部分完成的子任务是否足以支撑后续推理，通过分析下游计划对终极查询Q的依赖性，避免低效的完美主义搜索循环
    - 范围化计划修复：仅当信息不足时启动修复，先诊断故障范围（区分局部问题如错误查询与系统缺陷如无关推理轨迹），对前者微调子任务查询，对后者剪除无效分支并注入新逻辑子任务序列以重建解径

在迭代步骤t中，给定前一状态的任务计划Pₜ₋₁、事实列表Fₜ₋₁及新事实fₜ，SP的决策过程形式化为：

$$(Pₜ, Actionsₜ) = SP(Pₜ₋₁, Fₜ₋₁∪{fₜ}, Q) $$

其中Actionsₜ为待执行的具体子任务列表。SP的内部分派逻辑由新事实fₜ的完成度lₜ决定：
...
这种双模块设计使REAP既能高效处理一般进展，又具备从复杂故障中恢复并实时调整策略的强健智能。

#### 事实提取器(FE)

事实列表F作为整个推理过程中不断演进的知识基础，其可靠性与全面性至关重要。事实提取器模块旨在**从检索文档中提取高保真度的结构化事实**。为减少幻觉并增强可追溯性，我们要求大语言模型（LLM）不仅提供结论性陈述，还需阐明其推理过程并引用直接文本证据。

FE的核心功能在于不仅能识别直接答案，还能发现对后续推理步骤可能至关重要的潜在线索。对于子任务pₜ∈Actionsₜ中的子查询qₜ，FE的处理流程如下：

首先，检索函数从外部语料库C中获取一组相关文档Dₜ；接着，LLM $M_θ$处理这些文档以生成新的结构化事实对象fₜ。关键在于，模型的输入不仅包含当前查询和检索文档，还包含历史事实Fₜ₋₁。这种上下文条件使模型能进行更复杂的推理，例如共指消解及识别新信息与既有事实间的关系。生成过程表述为：

$$fₜ = M_θ(ExtractFact|qₜ, Dₜ, Fₜ₋₁)$$

事实对象fₜ采用结构化元组设计，以完整捕获提取信息的丰富性：
...
- eₜ⊆Dₜ是支撑陈述sₜ的直接文本证据片段集合

- rₜ是模型的推理过程，即从eₜ推导出sₜ的思维链解释

- lₜ是完成度等级，用于分类提取事实质量的标签，该等级对SP模块后续决策至关重要

核心陈述sₜ和完成度等级lₜ将直接指导SP模块的下一个规划周期。
...
其中任务集T={decomp, update, replan}，λ_task为任务权重（默认设为1），L_task为对应损失函数。该范式实现了从数据丰富的任务（分解器、更新器）向数据稀缺的重规划任务的有效知识迁移，显著增强了重规划器在复杂异常场景下的鲁棒性与准确性，从而全面提升REAP框架的智能水平。
...
作者在四个多跳数据集上对方法进行了评估：HotpotQA（Yang等人，2018年）、2WikiMultihopQA（Ho等人，2020年）、MuSiQue（Trivedi等人，2022b年）和Bamboogle（Press等人，2022年）。HotpotQA和2WikiMultihopQA是**领域内基准测试，因为它们的一部分训练集被用于模型训练**，而MuSiQue和Bamboogle则作为**领域外基准测试，用于评估方法的泛化能力**。对于前三个数据集，作者从每个验证集中随机抽取500个样本作为测试集。对于Bamboogle，由于它仅包含125个样本，我们使用整个测试集进行评估。在评估指标方面，我们采用了**Cover Exact Match（CEM）、F1分数和ACC†（LLM-as-gudge）**。详细的数据集和指标描述见附录。

#### 基线方法

- Naive方法直接生成答案而不进行任何检索。
- 标准RAG方法（Lewis等人，2020）执行单步检索，并将检索到的文档与问题拼接后生成答案。
- 此外，使用与本文方法相同的训练数据对标准RAG进行微调，并将其命名为FT-标准RAG。
- SuRe方法（Kim等人，2024）通过生成检索内容的条件摘要来对候选答案进行排序。
- IRCoT（Trivedi等人，2022a）、Iter-RetGen（Shao等人，2023）和SearChain（Xu等人，2024）将推理链与多轮检索相结合以形成推理轨迹。
- Search-R1（Jin等人，2025a）和R1-Searcher（Song等人，2025）采用模板引导提示、强化学习和摘要机制，实现证据与答案生成的更精细整合。

#### 实现细节

在实验中，REAP和其他未经微调的基线方法均**采用Llama-3.1-8B-Instruct作为生成器**，并通过**FlashRAG进行评估**。对于涉及微调的方法，使用作者提供的模型checkpoint。采用**CoRAG（Wang等人，2024b）提供的语料库**，该语料基于KILT提供的英文维基百科，包含约36万篇文章段。使用**e5-large-v2（Wang等人，2022）作为主检索器**，每个查询返回前5个结果。对于所有多轮方法，将最大迭代次数设为5。之后从**HotpotQA和WikiMultihopQA中随机选取7000个样本，并使用GPT-4（Achiam等人，2023）运行REAP来收集训练数据。**经过筛选后，**最终选定5556个样本作为训练集，其中2988个来自HotpotQA，2568个来自2WikiMultihopQA。**。

#### 主要结果

1. **迭代交互机制带来显著性能提升**。*与标准RAG方法相比*，REAP将HotpotQA数据集上的F1值从48.6%提升至68.0%，证实了迭代方法在多跳问答任务中的优越性。更重要的是，REAP相较FT-Standard RAG仍保持6.8%的F1值优势，表明性能提升不仅源于训练数据，更主要归功于本文提出的迭代推理与证据提取框架。
2. **REAP显著优于现有多轮次方法**。*在所有数据集上均超越FlashRAG系统中的最佳表现方法R1-Searcher*，其中HotpotQA提升4.6%，2WikiMultihopQA提升10.2%。这证明本文的方法有效增强了模型的推理与事实证据提取能力。
3. **模型展示了强大的泛化能力。**REAP仅在HotPotQA和2WikiMultihopQA上训练。但它不仅在这些领域内数据集上表现出色，还在领域外数据集上取得了最佳成绩，展现了其强大的泛化能力。这表明训练过程使模型能够学习推理和事实提取的基本技能，而不是简单地记忆生成模式，从而在面对未见数据时表现出稳健的性能。
...
对于SP模块，我们移除了专用于处理复杂场景的Re-Planner子模块（w/o replan）；对于FE模块，则分别禁用逻辑验证机制（w/o verify）和线索反馈机制（w/o clue）。结果如表2所示。

实验表明，完整REAP框架始终优于所有消融变体，验证了各机制的正向作用。

性能下降最显著的是w/o replan配置：HQA、MQA和BQA的F1分数分别降低3.1%、4.1%和3.6%。这一大幅下降（尤其在更复杂的MQA和BQA数据集上）凸显了Re-Planner动态修正推理轨迹的关键价值。

移除验证机制（w/o verify）会导致性能持续下降，其中BQA的F1分数骤降4.4%。这印证了在迭代过程中确保事实准确性的必要性——缺乏严格验证时，错误或虚构信息会持续传播，最终导致整个推理链崩溃。

禁用线索反馈机制（w/o clue）同样造成显著性能损失，例如HQA和WQA的F1分数分别下降3.4%和3.1%。这表明模型识别并利用潜在线索（而非仅依赖直接答案）的能力，对构建最终答案所需的完整事实基础至关重要。
...
结果显示了多任务微调方法的显著优势。ft-separate设置始终表现最差，在Bamboogle数据集上F1分数落后REAP达2.5%，在2Wiki数据集上落后2.4%。这表明孤立任务训练阻碍了模型学习规划模块间潜在关联和可泛化推理模式的能力，从而限制了其性能。

关键的是，采用精心设计的多任务微调策略的REAP框架，在所有基准测试中均取得最高分：在2Wiki上F1分数比ft-all高1.2%，在MuSiQue上高0.9%。这一最终改进证明，仅合并训练数据是不够的。我们的多任务微调策略能更有效地将鲁棒的规划能力迁移至数据稀缺但关键的重规划器模块，证实了方法论的有效性和合理性。
...
RAG通过从大型外部语料库或上下文中仅提取最相关部分（如新增知识或长文本）来运作，使LLMs能够处理更简短聚焦的输入，从而提升效率与输出质量。当前多数RAG方法采用单步检索机制，这种设置在"大海捞针"等相对简单任务中表现良好（Hsieh等人，2024）。但对于更复杂的问题，需要与上下文进行多步交互。**多步检索可视为基于搜索的推理形式**。

现有多种多步检索推理方法：一类是通过**检索信息构建知识图谱**（Ma等人，2025；Li等人，2024），这些方法在推理时往往较慢，因为LLM需为每个新输入处理全部上下文来构建图谱；另一类采用**LLM智能体，将RAG查询与LLM生成的指令交错执行**（Singh等人，2025；Anokhin等人，2024），这类系统对检索段落中的噪声或错误敏感，可能干扰后续查询生成。这表明需要对检索与生成组件进行联合优化。

近期出现的新方法通过**微调LLMs来提升与检索工具的交互效率**（Song等人，2025；Jin等人，2025；Chen等人，2025），虽然性能更优，但需要对LLM本身进行昂贵的微调，这使得它们难以应用于大型模型，也限制了大多数研究者和实践者的使用可行性。

在这项工作中，我们致力于开发一种基于强化学习的资源高效多步检索增强生成方法。不同于对大型语言模型进行微调，我们训练了一个能在文本块嵌入的潜在空间中直接执行检索的智能体。这使得我们能够利用基于价值的强化学习方法，学习到一个紧凑高效的模型。

本文提出的方法在长上下文常识推理、多跳问答以及需要处理高达1000万Tokens上下文的NIAH任务中取得了最先进的成果。该方法在Musique和HotpotQA（Yang等人，2018）等开放域问答基准测试中也展现出竞争力，同时与现有多步检索增强生成方法相比，训练和运行速度显著提升，成本大幅降低。主要贡献包括：

- 提出利用时序差分强化学习训练多步检索智能体的新方法。
- 在需要常识推理和超长上下文（高达1000万Tokens）NIAH任务的基准测试中取得最先进成果。
...
一种广受欢迎的方法是构建无需微调的LLM智能体，将现成的检索器与LLM结合，例如Search-o1。许多这类工作通过**构建大型知识图谱来进一步提升检索质量，虽然只需少量额外训练，但由于LLM需要处理整个上下文，推理速度极慢**，例如GraphReader、HippoRAG和AriGraph。'

另一类工作通过**微调LRM实现多步检索，使模型能够在长上下文推理中生成中间搜索查询**。首次应用这一思想的是IM-RAG，它使用PPO对LLM进行微调，同时冻结嵌入器。最近的论文如R1-Searcher、Search-R1、RAG-RL和ReSearcher通过采用GRPO扩展了这一方向。与这些冻结嵌入器并微调LLM的方法不同，本文的方法仅微调嵌入器，使其能够与任何规模的LLM（包括专有模型）配对，同时保持微调的高效性和低成本。

另一种方法是**利用LLM的反馈直接微调检索器本身**，如RePlug。这一方向与本文的方法最为相似，但RePlug未涉及多步推理或在此场景中使用强化学习。BeamRetriever通过训练用于BeamSearch式规划的重新排序器，在短上下文问答中取得了最先进的结果。相比之下，Q-RAG通过强化学习训练嵌入器，利用高效的向量相似性而非基于Transformer的轨迹评分，实现了更快的推理和更好的长上下文扩展性。

极长序列处理通过结合循环机制与Transformer架构的模型得以实现。
- Mamba系列的状态空间模型用结构化循环动态替代注意力机制，提供了线性时间扩展性和在长序列上的强大性能，尽管通常以较弱的上下文内学习和较不显著的token间交互为代价。
- 循环记忆Transformer（RMT）通过在固定大小的段之间传递记忆token引入段级循环，支持长达1000万token序列的问答。
- Titans将循环记忆训练视为元学习问题，展示了超过200万token的扩展能力。基于这一思想，
- ATLAS增加了记忆容量，取得了优于RMT和Titans的长上下文性能。
- 关联循环记忆Transformer（ARMT）在每一层采用准线性关联注意力，成为循环模型中长上下文得分最高的方法。我们的方法在超过100万token的上下文上优于所有这些模型，同时属于另一类方法。

LongRoPE2解决了位置编码瓶颈，通过RoPE重新缩放和混合窗口训练，将预训练LLM的有效上下文窗口扩展到128K token，同时保持短上下文性能。

## Chain-of-Retrieval Augmented Generation

### 研究背景

背景脉络：传统RAG->网络搜索增强的RAG->近来RAG研究聚焦于更优的文本嵌入模型->前沿研究已从传统的"单次检索+生成"范式演进至多步迭代检索与生成。

本研究另辟蹊径，不囿于小样本提示或专有模型蒸馏，而是创新性地训练LLM执行迭代检索与信息推理。

思维链说明引导模型分步推理能极大提升数学任务表现。思维树通过树状结构扩展CoT,使模型能更全面探索解空间。STaR通过自举技术生成中间状态以增强LLM推理能力。OpenAI o1[12]虽通过大规模强化学习在高级推理数据集展现优异计算扩展特性，但技术细节未公开。

在RAG领域，可通过增加检索文档或检索步骤扩展测试时计算。

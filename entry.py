...
async def main():
    # 创建AgentSystem
    client=MCPClient()
    await client.connect_to_server(r"mcp_server\server.py")
    manager = AgentManager()
    faissmanager_cfg=settings.agents.get("faissmanager",{})
    manager.register_agent("faissmanager",AsyncAgent(name="faissmanager",role=faissmanager_cfg.role,config=settings.llm))

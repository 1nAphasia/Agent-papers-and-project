...
class MCPClient:
    ...
    async def connect_to_server(self, server_script_path: str):
        ...
        print(
            ...
            [tool.name for tool in self.server_tools],
        )
    ...
    async def call_tools(self, tool_name, tool_args):
        ...
        logger.info(
            f"使用参数{str(tool_args)} 调用工具 {tool_name} \n 返回结果 {str(results)}"
        )
        return results
...
async def main():
    new_client = Client(
        r"C:\Users\79237\Desktop\学习\主线\Agent-papers-and-project\mcp_server\server.py"
    )
    async with new_client:
        await new_client.ping()

        print(await new_client.list_tools())
